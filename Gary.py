...
def to_small_caps(text):
    # Approximation: small-caps for lowercase where available
...
for name, fn in styles:
    styled = fn(user_text)
    st.markdown(f"**{name}**")
    st.markdown(f"<div style='font-size:1.6em; padding:6px 0'>{styled}</div>", unsafe_allow_html=True)
