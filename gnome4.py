...
def from_fraktur(text):
    ...
    mapping = {}
    for i, f in enumerate(fraktur[0]):
        mapping[f] = normal[i]
    for i, f in enumerate(fraktur[1]):
        mapping[f] = normal[i+26]
    return "".join(mapping.get(ch, ch) for ch in text)
...
st.write("Type English below to see Gnome fantasy text (Fraktur style), or paste Gnome/Gnlish below to decode:")

# --- Box 1: English to Gnome Fraktur ---
text = st.text_area("Enter English text:", "")

if text:
    gnomified = gnome_stylize_sentence(text)
    st.markdown("**Gnome/Goblin-ified (Fraktur):**")
    st.markdown(f"<div style='font-size:1.4em'>{to_fraktur(gnomified)}</div>", unsafe_allow_html=True)

# --- Box 2: Gnome to English ---
st.write("---")
gnome_input = st.text_area("Paste Gnome/Gnlish text here:", "")

if gnome_input:
    plain_gnome = from_fraktur(gnome_input)
    english_decoded = de_gnomeify_sentence(plain_gnome)
